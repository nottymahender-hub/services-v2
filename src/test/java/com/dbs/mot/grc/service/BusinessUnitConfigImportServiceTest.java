...
/**
 * Dedicated tests for {@link BusinessUnitConfigImportService} against H2 — the {@code BU_NUM}
 * keyed upsert, the export and the download/config metadata.
 */
@SpringBootTest
@ActiveProfiles("test")
class BusinessUnitConfigImportServiceTest {

    private static final String TABLE = "orl_biz_unit";
    private static final String FIXTURE = "biz-units.csv";

    @Autowired BusinessUnitConfigImportService service;
    ...
    void metadata_isExposedForTheController() {
        assertThat(service.configName()).isEqualTo("biz-units");
        assertThat(service.getDownloadFilename()).isEqualTo("orl_biz_unit.csv");
        assertThat(service.getDownloadHeaders()).contains("BU_NUM", "BU_NM", "LVL_OF_HIER");
    }
    ...
    void processAndImport_intoEmptyTable_insertsRows_stampingCreatedBy() throws Exception {
        ...
        // Empty table → every de-duplicated BU_NUM is an insert stamped with the operator.
    ...
    void processAndImport_reupload_updatesInsteadOfDuplicating() throws Exception {
        ...
        // Upsert keyed on BU_NUM: re-uploading the same keys must not grow the table, and the
        // update path stamps UPDATED_BY while carrying CREATED_BY forward.
